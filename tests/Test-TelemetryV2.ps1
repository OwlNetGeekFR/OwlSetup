...
$model = Get-Content (Join-Path $root 'OwlSetup-Dashboard\Models\DashboardModels.cs') -Raw -Encoding UTF8
$dashboard = Get-Content (Join-Path $root 'OwlSetup-Dashboard\Pages\Index.cshtml') -Raw -Encoding UTF8
$dashboardCode = Get-Content (Join-Path $root 'OwlSetup-Dashboard\Pages\Index.cshtml.cs') -Raw -Encoding UTF8
